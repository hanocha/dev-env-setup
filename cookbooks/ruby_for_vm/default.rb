...
execute "add rbenv settings" do
  command %|
    echo 'export RBENV_ROOT=/home/#{node.user.name}/.rbenv' >> /home/#{node.user.name}/.bashrc
    echo 'set -x PATH $RBENV_ROOT/bin $PATH' >> /home/#{node.user.name}/.bashrc
    echo 'set -x PATH $RBENV_ROOT/shims $PATH' >> /home/#{node.user.name}/.bashrc
    echo 'eval (rbenv init -)' >> /home/#{node.user.name}/.bashrc
|
